...
void calculate_score(Data *person,Data *User,int data_amount);
int judge_mode(Data *User);                             // 回傳使用者性向狀況
void sexuality_score(Data *person,Data *User,int data_amount);
void age_score(Data *person,Data *User,int data_amount);
void area_score(Data *person,Data *User,int data_amount);
void hobby_score(Data *person,Data *User,int data_amount);

int main(){
    int data_amount = 0;
    Data person[1000];
    data_amount = read_file(person);
    calculate_score(person,User,data_amount);
...
void calculate_score(Data *person,Data *User,int data_amount){
    sexuality_score(person,User,data_amount);
    age_score(person,User,data_amount);
    area_score(person,User,data_amount);
    hobby_score(person,User,data_amount);
}
...
void sexuality_score(Data *person,Data *User,int data_amount){     // 性向對了+500
...
void age_score(Data *person,Data *User,int data_amount){           // 年齡對了+100
...
void area_score(Data *person,Data *User,int data_amount){          // 距離差0加100,差1加90,差2加80...10以上不加了
    for(int i = 0;i < data_amount;i++){
...
void hobby_score(Data *person,Data *User,int data_amount){         //每對一個+20
    for(int i = 0;i < 5;i++){       
